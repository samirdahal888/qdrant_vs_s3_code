"""Test 04: Filter — Combined AND (genre=Sci-Fi + year>=2010 + rating>7.5) — Both."""

import time

from qdrant_client import models

from core.clients import get_clients
from core.config import QDRANT_COLLECTION, S3V_BUCKET_NAME, S3V_INDEX_NAME
from core.embeddings import generate_query_embedding


def run():
    qc, sc = get_clients()
    qvec = generate_query_embedding("best sci-fi movies")

    print("=" * 60)
    print("TEST 04: Filter — Combined AND (Sci-Fi + year>=2010 + rating>7.5)")
    print("=" * 60)

    # Qdrant: must = AND
    t0 = time.perf_counter()
    q_res = qc.query_points(
        QDRANT_COLLECTION,
        query=qvec,
        limit=5,
        with_payload=True,
        query_filter=models.Filter(
            must=[
                models.FieldCondition(
                    key="genre", match=models.MatchValue(value="Sci-Fi")
                ),
                models.FieldCondition(key="year", range=models.Range(gte=2010)),
                models.FieldCondition(key="rating", range=models.Range(gt=7.5)),
            ]
        ),
    )
    q_ms = (time.perf_counter() - t0) * 1000

    # S3 Vectors: $and
    t0 = time.perf_counter()
    s_res = sc.query_vectors(
        vectorBucketName=S3V_BUCKET_NAME,
        indexName=S3V_INDEX_NAME,
        queryVector={"float32": qvec},
        topK=5,
        filter={
            "$and": [
                {"genre": "Sci-Fi"},
                {"year": {"$gte": 2010}},
                {"rating": {"$gt": 7.5}},
            ]
        },
        returnDistance=True,
        returnMetadata=True,
    )
    s_ms = (time.perf_counter() - t0) * 1000

    print(f"\nQdrant ({q_ms:.0f}ms):")
    for p in q_res.points:
        print(
            f"  {p.payload['title']} — genre={p.payload['genre']}, year={p.payload['year']}, rating={p.payload['rating']}"
        )

    print(f"\nS3 Vectors ({s_ms:.0f}ms):")
    for v in s_res["vectors"]:
        m = v["metadata"]
        print(
            f"  {m['title']} — genre={m['genre']}, year={m['year']}, rating={m['rating']}"
        )


if __name__ == "__main__":
    run()
